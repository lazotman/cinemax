...
.season-btn{background:${G.card};border:1px solid ${G.border};color:${G.muted};padding:7px 14px;border-radius:6px;font-size:.78rem;cursor:pointer;transition:all .18s;font-family:${G.body}}
.season-btn.active,.season-btn:hover{background:${G.accent};border-color:${G.accent};color:#fff}
@media(max-width:768px){.detail-main{flex-direction:column}.detail-poster{width:140px!important}}
`;
...
export default function TVPage({ params }) {
  ...
  const totalEpisodes = data.seasons?.find(s => s.season_number === season)?.episode_count || 10;
    ...
    <>
      ...
      <nav className="nav">
        <div className="nav-inner">
          ...
          <span style={{ background: "#0066ff", color: "#fff", fontSize: ".62rem", fontWeight: 700, padding: "3px 8px", borderRadius: 3, letterSpacing: 1 }}>TV SHOW</span>
        </div>
      </nav>
      ...
      <div style={{ position: "relative", minHeight: 420, overflow: "hidden" }}>
        ...
        <div style={{ position: "relative", zIndex: 1, maxWidth: 1400, margin: "0 auto", padding: "clamp(40px,6vw,80px) clamp(14px,4vw,56px)" }}>
          <div className="detail-main" style={{ display: "flex", gap: "clamp(20px,4vw,48px)", alignItems: "flex-start" }}>
            {poster && <img src={poster} alt={data.name} className="detail-poster" style={{ width: "clamp(140px,18vw,240px)", flexShrink: 0, borderRadius: 10, boxShadow: "0 20px 60px rgba(0,0,0,.8)", border: `1px solid ${G.border}` }} />}
            <div style={{ flex: 1, minWidth: 260 }} className="fade-up">
              <h1 style={{ fontFamily: G.font, fontSize: "clamp(1.8rem,5vw,3.5rem)", letterSpacing: 2, lineHeight: 1.05, marginBottom: 12 }}>{data.name}</h1>
              <div style={{ display: "flex", gap: 10, flexWrap: "wrap", alignItems: "center", marginBottom: 14 }}>
                <span className="rating">★ {Number(data.vote_average || 0).toFixed(1)}</span>
                {year && <span className="tag">{year}</span>}
                <span className="tag">{seasons} Season{seasons > 1 ? "s" : ""}</span>
              ...
              <p style={{ color: G.soft, fontSize: "clamp(.875rem,1.5vw,1rem)", lineHeight: 1.8, marginBottom: 20, maxWidth: 600 }}>{data.overview}</p>

              {/* Season/Episode selector */}
              <div style={{ marginBottom: 16 }}>
                <p style={{ color: G.muted, fontSize: ".75rem", marginBottom: 8, textTransform: "uppercase", letterSpacing: 1 }}>Season</p>
                <div style={{ display: "flex", gap: 8, flexWrap: "wrap", marginBottom: 12 }}>
                  {Array.from({ length: seasons }, (_, i) => i + 1).map(s => (
                    <button key={s} className={`season-btn ${season === s ? "active" : ""}`} onClick={() => { setSeason(s); setEpisode(1); }}>S{s}</button>
                  ))}
                </div>
                <p style={{ color: G.muted, fontSize: ".75rem", marginBottom: 8, textTransform: "uppercase", letterSpacing: 1 }}>Episode</p>
                <div style={{ display: "flex", gap: 8, flexWrap: "wrap", maxHeight: 100, overflowY: "auto" }}>
                  {Array.from({ length: Math.min(totalEpisodes, 30) }, (_, i) => i + 1).map(e => (
                    <button key={e} className={`season-btn ${episode === e ? "active" : ""}`} onClick={() => setEpisode(e)}>E{e}</button>
                  ))}
                </div>
              </div>

              <div style={{ display: "flex", gap: 10, flexWrap: "wrap" }}>
                <button className="btn btn-red" onClick={() => setPlaying(true)}>▶ Watch S{season}E{episode} Free</button>
                <a href="/" className="btn btn-ghost" style={{ textDecoration: "none" }}>🏠 More Shows</a>
              </div>
            </div>
          </div>
        </div>
      ...
      <div style={{ maxWidth: 1400, margin: "0 auto", padding: "0 clamp(14px,4vw,56px) 60px" }}>
          ...
          <div className="cast-grid">
            {cast.map(c => (
              <div key={c.id} className="cast-card">
                <img className="cast-photo" src={c.profile_path ? `${IMG}/w185${c.profile_path}` : `https://via.placeholder.com/72x72/111820/7a8fa6?text=?`} alt={c.name} loading="lazy" />
              ...
              <div style={{ maxWidth: 800 }}>
                <div style={{ borderRadius: 10, overflow: "hidden", background: "#000" }}>
                  <div style={{ position: "relative", paddingTop: "56.25%" }}>
                    <iframe src={`https://www.youtube.com/embed/${trailer.key}?rel=0`} style={{ position: "absolute", inset: 0, width: "100%", height: "100%", border: "none" }} allowFullScreen title={trailer.name} />
                  </div>
                </div>
              </div>
        ...
        {tab === "related" && (
          <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fill,minmax(150px,1fr))", gap: 14 }}>
            {related.map(m => (
              <a key={m.id} href={`/tv/${m.id}`} style={{ textDecoration: "none" }}>
                <div className="movie-card">
                  <div style={{ position: "relative", aspectRatio: "2/3" }}>
                    <img src={`${IMG}/w342${m.poster_path}`} alt={m.name} style={{ width: "100%", height: "100%", objectFit: "cover" }} loading="lazy" />
        ...
        <div style={{ marginTop: 48, padding: 24, background: G.card, borderRadius: 10, border: `1px solid ${G.border}` }}>
          <h2 style={{ fontFamily: G.font, fontSize: "1.3rem", letterSpacing: 1, marginBottom: 12 }}>
            Watch {data.name} ({year}) Online Free — All Seasons
          </h2>
          <p style={{ color: G.muted, fontSize: ".875rem", lineHeight: 1.8 }}>
            ...
            {genres ? ` Genre: ${genres}.` : ""}
            {" "}Stream {data.name} in HD quality. {data.number_of_seasons} seasons, {data.number_of_episodes} episodes available.
          </p>
        </div>
      </div>
      ...
      {playing && (
        <div className="modal-bg" onClick={() => setPlaying(false)}>
          <div style={{ width: "100%", maxWidth: 1000, position: "relative" }} onClick={e => e.stopPropagation()}>
            <button onClick={() => setPlaying(false)} style={{ position: "absolute", top: -42, right: 0, background: "none", border: "none", color: G.text, fontSize: ".85rem", fontWeight: 600, cursor: "pointer", padding: 8, display: "flex", alignItems: "center", gap: 6 }}>✕ Close Player</button>
            <div className="iframe-wrap">
              <div className="iframe-ratio">
                <iframe src={`https://vidsrc.to/embed/tv/${id}/${season}-${episode}`} allowFullScreen referrerPolicy="origin" title={data.name} />
              </div>
              <div style={{ background: G.surface, padding: "14px 18px" }}>
                <span style={{ fontFamily: G.font, fontSize: "1.2rem", letterSpacing: 1 }}>{data.name}</span>
                <span className="tag" style={{ marginLeft: 10 }}>S{season} E{episode}</span>
              </div>
            </div>
          </div>
        </div>
